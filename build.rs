...
fn main() {
    ...
    if let Ok(rd) = fs::read_dir(render_dir) {
            ...
            .filter(|e| e.file_type().map(|t| t.is_file()).unwrap_or(false))
        {
            ...
            if let Some(node) = nodes.iter().find(|n| snake(&n.name) == hname) {
    ...
    // the handler `mod` list lives in the SOURCE tree (mod path resolution);
    // OUT_DIR holds only the dispatch fn.
    fs::write(
    ...
    fs::write(format!("{out}/render_ext_gen.rs"), rgen).unwrap();
}
