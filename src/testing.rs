...
pub fn check_perl_critic_available() -> bool {
    ...
    let strawberry_perl = perl_executable();
...
pub fn run_perl_critic_brutal(perl_code: &str) -> Result<String, String> {
    ...
    if !check_perl_critic_available() {
        return Ok("Perl::Critic not found in PATH. Skipping Perl::Critic checks.".to_string());
    }
                ...
                if tidy_output.status.success() {
                    ...
                    if tidy_stdout.contains("Original:") && tidy_stdout.contains("Tidied:") {
                        ...
                        let original_normalized =
                            original_trimmed.replace("\r\n", "\n").replace("\r", "\n");
                        let tidied_normalized =
                            tidied_trimmed.replace("\r\n", "\n").replace("\r", "\n");
...
pub fn find_uses_of_system() {
    println!(
        "Scanning examples/* for shell scripts and finding 'system' usage in Perl translations..."
    );
...
pub fn test_file_equivalence_with_critic(
    lang: &str,
    filename: &str,
    ...
    let (tmp_file, run_cmd) = match lang {
        "perl" => {
            ...
            if let Err(violation_msg) = check_perl_must_not_contain(&shell_content, &code) {
                return Err(format!(
                    "PERL_MUST_NOT_CONTAIN constraint violation in {}:\n{}",
            ...
            (
                tmp_str.clone(),
                vec![perl_executable().to_string(), tmp_str],
            )
        }
        _ => {
            return Err(format!("Unsupported language for --test-file: {}", lang));
        }
    };
    ...
    let translated_output = {
        if lang == "rust" {
            ...
            let mut child = match Command::new(&abs_bin)
                .stdout(Stdio::piped())
                .stderr(Stdio::piped())
                .spawn()
            {
            ...
            let mut child = match cmd.stdout(Stdio::piped()).stderr(Stdio::piped()).spawn() {
                ...
                Err(e) => {
                    cleanup_tmp(lang, &tmp_file);
                    return Err(format!("Failed to run translated program: {}", e));
                }
            };
            let start = std::time::Instant::now();
            let out = loop {
    ...
    let shell_stderr = String::from_utf8_lossy(&shell_output.stderr)
        ...
        .trim()
        .to_string();
    let trans_stdout = String::from_utf8_lossy(&translated_output.stdout)
        .to_string()
        .replace("\r\n", "\n")
        .trim()
        .to_string();
    let trans_stderr = String::from_utf8_lossy(&translated_output.stderr)
        .to_string()
        .replace("\r\n", "\n")
        .trim()
        .to_string();
    ...
    println!(
        "Shell exit: {} | Translated exit: {}",
        shell_output.status, translated_output.status
    );
    ...
    if shell_stdout != trans_stdout {
        return Err(format!(
            "STDOUT mismatch (lang: {}, file: {}): stdout differs even with matching exit codes",
            lang, filename
        ));
    }
    ...
    if shell_stderr != trans_stderr {
        return Err(format!(
            "STDERR mismatch (lang: {}, file: {}): stderr differs",
            lang, filename
        ));
    }

    if !shell_success {
        // Both programs failed - stdout matched, stderr is informational only.
        println!(
            "Both programs failed with matching stdout (lang: {}, file: {})",
            lang, filename
        );
    } else {
        // Both programs succeeded - stdout matched, stderr is informational only.
        println!(
            "Both programs succeeded with matching stdout (lang: {}, file: {})",
            lang, filename
        );
    }
...
pub fn test_file_equivalence_detailed(
    lang: &str,
    filename: &str,
    ast_options: Option<AstFormatOptions>,
) -> Result<TestResult, String> {
    test_file_equivalence_detailed_with_critic(lang, filename, ast_options, false)
}

pub fn test_file_equivalence_detailed_with_critic(
    lang: &str,
    filename: &str,
    ast_options: Option<AstFormatOptions>,
    enable_perl_critic: bool,
    ...
    if lang == "perl" && cached_perl_code.is_some() && tmp_file.is_empty() {
        ...
        if let Err(e) =
            shared_utils::SharedUtils::write_utf8_file(&output_filename, &translated_code)
        {
            return Err(format!(
                "Failed to write Perl file to {}: {}",
                output_filename, e
            ));
        }
        ...
        run_cmd = vec![perl_executable().to_string(), tmp_str];
    }
    ...
    if shell_output.is_none() {
        ...
        if interactive_scripts.contains(&script_name) {
            ...
            eprintln!(
                "DEBUG: Shell script completed in {:.2}s",
                shell_duration.as_secs_f64()
            );
    ...
    if cached_perl_code.is_none() {
        ...
        let commands = match Parser::new(&shell_content).parse() {
            ...
            Err(e) => {
                ...
                while !lexer.is_eof() && token_count < 1000 {
                    ...
                    if let Some(token) = lexer.peek() {
                        ...
                        lexer_output.push_str(&format!(
                            "{:?}('{}') at {}:{}; ",
                            token, token_text, line, col
                        ));
                        lexer.next(); // Advance to next token
                        token_count += 1;
                    } else {
        ...
        eprintln!(
            "DEBUG: Perl code generated, length: {} characters",
            translated_code.len()
        );
    ...
    if lang == "perl" && enable_perl_critic {
        ...
        if let Ok(_) = std::fs::write(&temp_file_tidy, &translated_code) {
            if let Ok(tidy_output) = std::process::Command::new(perl_executable())
                .arg("test_wrapper_minimal.pl")
                    ...
                    if let Some(tidied_start) = tidy_stdout.find("Tidied:") {
                        ...
                        if false && !tidied_content.is_empty() && tidied_content != &translated_code
                        {
                            ...
                            return Ok(TestResult {
                                ...
                                _lexer_output: String::new(),
                                failure_reason:
                                    "Code is not tidy - PerlTidy formatting differences detected"
                                        .to_string(),
                                shell_duration: std::time::Duration::from_secs(0),
                                translated_duration: std::time::Duration::from_secs(0),
                            });
    ...
    let (translated_output, translated_duration, timed_out) = {
        if lang == "rust" {
            ...
            if lang == "perl" {
                ...
                eprintln!(
                    "DEBUG: Perl code length: {} characters",
                    translated_code.len()
                );
                eprintln!(
                    "DEBUG: First 200 characters of Perl code:\n{}",
            ...
            execute_with_timeout(OperationType::PerlExecution, move || {
                ...
                let duration = start.elapsed();
                eprintln!(
                    "DEBUG: Perl execution completed, duration: {:?}, timed_out: {}",
                    duration, timed_out
                );
    ...
    // Normalize and compare
    let shell_stdout = String::from_utf8_lossy(&shell_output_result.stdout)
        .to_string()
        .replace("\r\n", "\n")
        .trim()
        .to_string();
    let shell_stderr = String::from_utf8_lossy(&shell_output_result.stderr)
        .to_string()
        .replace("\r\n", "\n")
        .trim()
        .to_string();
    ...
    // Determine success based on exit status and stdout matching.
    // Stderr is still captured and shown for diagnostics, but it is not
    // required for the default fail-path success criteria.
    ...
    eprintln!(
        "DEBUG: Shell stdout length: {}, Translated stdout length: {}",
        shell_stdout.len(),
        trans_stdout.len()
    );
    eprintln!(
        "DEBUG: Shell stderr length: {}, Translated stderr length: {}",
        shell_stderr.len(),
        trans_stderr.len()
    );
    eprintln!(
        "DEBUG: Shell exit: {}, Translated exit: {}",
        shell_output_result.status.code().unwrap_or(-1),
        translated_output.status.code().unwrap_or(-1)
    );
    eprintln!(
        "DEBUG: stdout_match: {}, stderr_match: {}, exit_code_match: {}",
        stdout_match, stderr_match, exit_code_match
    );
...
pub fn test_all_examples() {
    ...
    let generators: Vec<_> = all_generators
        .into_iter()
        .filter(|g| {
    ...
    for generator in &generators {
        ...
        println!(
            "{:<8}: {}/{} passed ({:.1}%) {}",
            generator, gen_passed, gen_total, percentage, status
        );
    }
    println!("{}", "=".repeat(80));
    ...
    println!("{}", "=".repeat(50));
    for example in &examples {
        let example_results: Vec<_> = results.iter().filter(|(e, _, _, _)| e == example).collect();
        let example_passed = example_results
            .iter()
            .filter(|(_, _, success, _)| *success)
            .count();
    ...
    if passed_tests < total_tests {
        println!(
            "FINAL RESULT: {} out of {} tests PASSED ({:.1}%)",
            passed_tests,
            total_tests,
            (passed_tests as f64 / total_tests as f64) * 100.0
        );
...
pub fn test_all_examples_next_fail(
    generators: &[String],
    ...
    let generators: Vec<_> = generators
        ...
        .collect();
    eprintln!(
        "DEBUG: Generator filtering completed in {:.2}ms",
    ...
    eprintln!(
        "DEBUG: Found {} examples in {:.2}ms",
        examples.len(),
        examples_start.elapsed().as_millis()
    );
    ...
    for generator in &generators {
        for (example_index, example) in examples.iter().enumerate() {
            ...
            if current_test % 5 == 0 {
                eprintln!(
                    "DEBUG: Progress: {}/{} tests completed ({:.1}%)",
                    current_test,
                    total_tests,
            ...
            print!(
                "\rTest {}/{}: {} with {:<8} ",
                current_test, total_tests, example_name, generator
            );
            io::stdout().flush().unwrap();

            // Run the actual test with fine-grained timeouts and debug freeze support
            eprintln!(
                "DEBUG: Starting test execution for {} with generator {}",
                example, generator
            );
            ...
            if is_execution_frozen() {
                eprintln!(
                    "DEBUG: Execution is frozen for debugging. Test {} will be skipped.",
                    example
                );
                print!("⏸️");
                continue;
            ...
            let test_result = test_file_equivalence_detailed_with_critic(
                &generator_clone,
                &example_clone,
                Some(AstFormatOptions::default()),
                enable_perl_critic,
            );

            eprintln!(
                "DEBUG: Test execution completed in {:.2}ms",
                individual_test_start.elapsed().as_millis()
            );
            ...
            match test_result {
                    ...
                    if result.success {
                        ...
                        if generator.as_str() == "perl" {
                            ...
                            if let Ok(_) = std::fs::write(&temp_file, &result.translated_code) {
                                if let Ok(tidy_output) =
                                    std::process::Command::new(perl_executable())
                                        .arg("test_wrapper_minimal.pl")
                                        .arg(&temp_file_str)
                                        .output()
                                {
                                    if tidy_output.status.success() {
                                        ...
                                        if tidy_stdout.contains("Original:")
                                            && tidy_stdout.contains("Tidied:")
                                        {
                                            println!("\n{}", "=".repeat(80));
                        ...
                        // Check PERL_MUST_CONTAIN constraints
                        if generator.as_str() == "perl" {
                            if let Err(violation_msg) = check_perl_must_contain(
                        ...
                        if generator.as_str() == "perl" {
                            if let Err(violation_msg) = check_perl_must_not_contain(
                                &result.original_code,
                                &result.translated_code,
                            ) {
                                println!("\n{}", "=".repeat(80));
                        ...
                        if let Err(violation_msg) =
                            check_ast_must_contain(&result.original_code, &result.ast)
                        {
                            println!("\n{}", "=".repeat(80));
                        ...
                        if let Err(violation_msg) =
                            check_ast_must_not_contain(&result.original_code, &result.ast)
                        {
                            println!("\n{}", "=".repeat(80));
                        ...
                        if result.shell_stderr != result.translated_stderr {
                            ...
                            println!(
                                ...
                                generate_unified_diff(
                                    &result.shell_stderr,
                                    &result.translated_stderr,
                                    "shell_stderr",
                                    &format!("{}_stderr", generator)
                                )
                            );
                        } else {
                ...
                Err(e) => {
                    ...
                    println!(
                        "Writing test count {} to first_n_tests_passed.txt (parsing error)",
                        passed_tests
                    );
                    println!(
                        "Current working directory: {:?}",
...
fn determine_failure_type(error_msg: &str) -> &'static str {
    if error_msg.contains("Failed to read") {
        ...
        "canparse"
    } else if error_msg.contains("Failed to run translated program")
        || error_msg.contains("Failed to run compiled")
    {
        "cangenerate"
    } else if error_msg.contains("STDOUT mismatch")
        || error_msg.contains("STDERR mismatch")
        || error_msg.contains("Exit status mismatch")
    {
        "cangenerate"
    } else {
        "failed"
...
fn write_results_to_files(
    ...
    if let Err(e) = fs::write("results/equivalent.txt", equivalent.join("\n")) {
        ...
        println!(
            "Wrote {} equivalent tests to results/equivalent.txt",
            equivalent.len()
        );
    }
    ...
    if let Err(e) = fs::write("results/cangenerate.txt", cangenerate.join("\n")) {
        ...
        println!(
            "Wrote {} generation-failed tests to results/cangenerate.txt",
            cangenerate.len()
        );
    }
    ...
    if let Err(e) = fs::write("results/canparse.txt", canparse.join("\n")) {
        ...
        println!(
            "Wrote {} parse-failed tests to results/canparse.txt",
            canparse.len()
        );
    }
    ...
    if let Err(e) = fs::write("results/canlex.txt", canlex.join("\n")) {
        ...
        println!(
            "Wrote {} lex-failed tests to results/canlex.txt",
            canlex.len()
        );
    }
    ...
    if let Err(e) = fs::write("results/failed.txt", failed.join("\n")) {
        ...
        println!(
            "Wrote {} lex-failed tests to results/failed.txt",
            failed.len()
        );
    }
}
...
pub fn test_all_examples_next_fail_unlimited(
    generators: &[String],
    test_prefix: Option<String>,
    enable_perl_critic: bool,
) {
