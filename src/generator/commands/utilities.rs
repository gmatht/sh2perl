...
pub fn source_safe_perl_string_expr(s: &str) -> String {
    ...
    while i < s.len() {
        ...
        let prev_char_ok = || -> bool { i == 0 || !s[..i].chars().last().map_or(false, |c| c.is_alphanumeric() || c == '_') };
        if s[i..].starts_with("system") && prev_char_ok()
            && (i + 6 >= s.len() || !s[i+6..].chars().next().map_or(false, |c| c.is_alphanumeric() || c == '_'))
        {
            if start < i {
