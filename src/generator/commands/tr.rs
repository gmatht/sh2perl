...
pub fn generate_tr_command(
    ...
    generate_tr_command_with_output(
        generator,
        cmd,
        input_var,
        command_index,
        linebyline,
        &format!("tr_result_{}", command_index),
    )
}

pub fn generate_tr_command_for_substitution(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    command_index: &str,
) -> String {
    generate_tr_command_with_output_for_substitution(
        generator,
        cmd,
        input_var,
        command_index,
...
pub fn generate_tr_command_with_output(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    command_index: &str,
    ...
    if linebyline {
        generate_tr_linebyline_impl_with_output(
            generator,
            cmd,
            input_var,
            command_index,
...
pub fn generate_tr_command_with_output_for_substitution(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    command_index: &str,
    ...
    generate_tr_buffered_impl_with_output_for_substitution(
        generator,
        cmd,
        input_var,
        command_index,
...
fn generate_tr_linebyline_impl(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    _command_index: &str,
...
fn generate_tr_linebyline_impl_with_output(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    _command_index: &str,
...
fn generate_tr_buffered_impl(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    command_index: &str,
) -> String {
    generate_tr_buffered_impl_with_output(
        generator,
        cmd,
        input_var,
        command_index,
...
fn generate_tr_buffered_impl_with_output(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    _command_index: &str,
    ...
    if delete_mode && args.len() >= 1 {
        ...
        output.push_str(&format!(
            "for my $char ( split //msx, $input_{} ) {{\n",
            unique_id
        ));
        output.push_str(&format!(
            "    if ( (index $set1_{}, $char) == -1 ) {{\n",
            unique_id
        ));
        ...
        output.push_str(&format!(
            "my $expanded_set1_{} = $set1_{};\n",
            unique_id, unique_id
        ));
        output.push_str(&format!(
            "my $expanded_set2_{} = $set2_{};\n",
            unique_id, unique_id
        ));
        output.push_str(&format!("# Handle a-z range in set1\n"));
        output.push_str(&format!(
            "if ($expanded_set1_{} =~ /a-z/msx) {{\n",
            unique_id
        ));
        output.push_str(&format!(
            "    $expanded_set1_{} =~ s/a-z/abcdefghijklmnopqrstuvwxyz/msx;\n",
            unique_id
        ));
        output.push_str(&format!("}}\n"));
        output.push_str(&format!("# Handle A-Z range in set1\n"));
        ...
        output.push_str(&format!(
            "    my $pos_{} = index $expanded_set1_{}, $char;\n",
            unique_id, unique_id
        ));
        output.push_str(&format!(
            "    if ( $pos_{} >= 0 && $pos_{} < length $expanded_set2_{} ) {{\n",
            unique_id, unique_id, unique_id
        ));
        output.push_str(&format!(
            "        ${} .= substr $expanded_set2_{}, $pos_{}, 1;\n",
...
fn generate_tr_buffered_impl_with_output_for_substitution(
    generator: &mut Generator,
    cmd: &SimpleCommand,
    input_var: &str,
    _command_index: &str,
