...
pub fn generate_diff_command(generator: &mut Generator, cmd: &SimpleCommand, _input_var: &str, _command_index: usize, _is_final_command: bool) -> String {
    ...
    if !args.is_empty() {
        ...
        output.push_str(&generator.indent());
        output.push_str("close $diff_fh; # diff command may exit with non-zero status, so ignore close errors\n");
        output.push_str(&generator.indent());
        ...
        generator.indent_level += 1;
        output.push_str(&generator.indent());
        output.push_str("carp \"Cannot execute diff command: $OS_ERROR\";\n");
        output.push_str(&generator.indent());
