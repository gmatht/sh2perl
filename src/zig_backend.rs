...
use crate::ir::{ArithAst, BinOpKind, IrExpr, IrProgram, IrStmt, IrType, InterpPart};
...
const ZIG_RESERVED: &[&str] = &[
    ...
    "std", "i64", "u8", "usize", "bool", "void", "noreturn", "true", "false",
    "null", "undefined", "main", "stdout",
    "sh2TODO", "sh2ToInt", "sh2IntStr", "sh2Truthy", "sh2StrCat", "sh2B2S",
];
...
impl Render {
    ...
    fn expr_num(&mut self, e: &IrExpr) -> String {
        match e {
            ...
            IrExpr::Bool(b) => {
                if *b { "@intFromBool(true)".into() } else { "@intFromBool(false)".into() }
            }
            IrExpr::Call { func, args } if func == "getVar" => {
    ...
    fn parts_of(&mut self, e: &IrExpr) -> Vec<(char, String)> {
        match e {
            ...
            IrExpr::Bool(b) => {
                if *b { vec![('b', "true".into())] } else { vec![('b', "false".into())] }
            }
            IrExpr::BinOp { .. } | IrExpr::Call { .. } | IrExpr::Capture { .. }
            | IrExpr::Index { .. } | IrExpr::Ternary { .. } | IrExpr::DefinedOr { .. }
            | IrExpr::Regex { .. } | IrExpr::MethodCall { .. } => vec![self.part_spec(e)],
            other => {
        ...
        match toks.as_slice() {
            [a, op, b] => {
                ...
                if let Some(o) = num_op {
                    ...
                    let va = if na { va } else { self.need_toint = true; format!("sh2ToInt({va})") };
                    let vb = if nb { vb } else { self.need_toint = true; format!("sh2ToInt({vb})") };
                    Some(format!("({va} {o} {vb})"))
                } else if let Some(o) = str_op {
                    let (va, na) = self.test_operand(a);
                    let (vb, nb) = self.test_operand(b);
                    let va = if na { self.need_intstr = true; format!("sh2IntStr({va})") } else { va };
                    let vb = if nb { self.need_intstr = true; format!("sh2IntStr({vb})") } else { vb };
                    if o == "==" {
            ...
            [flag, v] if *flag == "-n" => {
                ...
                let vv = if nv { self.need_intstr = true; format!("sh2IntStr({vv})") } else { vv };
    ...
    fn stmt(&mut self, s: &IrStmt) {
        match s {
            ...
            IrStmt::Die { .. } | IrStmt::Warn { .. } | IrStmt::Return(_)
            | IrStmt::SetChildError(_) | IrStmt::Require(_) | IrStmt::RawText(_)
            | IrStmt::Case { .. } | IrStmt::Redirect { .. } | IrStmt::Function { .. }
    ...
    fn program(&mut self, prog: &IrProgram) {
        ...
        for v in &written {
            if !self.read.contains(v) {
                let m = self.mangle.get(v).cloned().unwrap_or_else(|| self.zig_ident(v));
        ...
        if self.need_intstr {
            ...
            self.emit("    return std.fmt.allocPrint(std.heap.page_allocator, \"{d}\", .{n}) catch \"\";");
            self.emit("}");
        }
        if self.need_truthy {
            self.emit("");
        ...
        if self.need_stdout {
            self.emit("");
            self.out.push("    const stdout = std.io.getStdOut().writer();".to_string());
        }
...
fn collect_written(stmts: &[IrStmt], out: &mut BTreeSet<String>) {
    for s in stmts {
        match s {
            ...
            IrStmt::Block(b) | IrStmt::Subshell(b) | IrStmt::Background(b) => collect_written(b, out),
...
mod tests {
    ...
    fn assigns_and_echo() {
        ...
        assert!(out.contains("try stdout.print(\"{s}{d}\\n\", .{\"x is \", x});"), "{out}");
