...
impl PerlGenerator {
    pub fn generate_test_expression(&mut self, test_expr: &TestExpression) -> String {
        ...
        if expr.contains(" =~ ") {
            ...
            if parts.len() == 2 {
                ...
                let parsed_expr1 = self.generate_test_expression(&TestExpression {
                    expression: expr1.to_string(),
                    modifiers: modifiers.clone(),
                });
                let parsed_expr2 = self.generate_test_expression(&TestExpression {
            ...
            let parsed_subexpr = self.generate_test_expression(&TestExpression {
                expression: subexpr,
                modifiers: modifiers.clone(),
            });
    ...
    pub fn generate_test_command(&mut self, cmd: &SimpleCommand, output: &mut String) {
        ...
        if cmd.name == "test" || cmd.name == "[" {
            ...
            let test_expr = self.convert_test_args_to_expression(&cmd.args);
            let result = self.generate_test_expression(&test_expr);
    ...
    fn convert_test_args_to_expression(&self, args: &[Word]) -> TestExpression {
