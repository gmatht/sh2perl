...
impl MirCommand {
    ...
    fn expand_brace_expansion(expansion: &BraceExpansion) -> Vec<MirWord> {
        ...
        if expansion.items.len() == 1 {
            ...
            match item {
                BraceItem::Literal(s) => {
                    expanded_words.push(MirWord::from_ast_word(Word::literal(format!("{}{}{}", prefix, s, suffix))));
                }
                BraceItem::Range(range) => {
                    ...
                    if let (Ok(start_num), Ok(end_num)) = (range.start.parse::<i32>(), range.end.parse::<i32>()) {
                        ...
                        while if step > 0 { current <= end_num } else { current >= end_num } {
                            expanded_words.push(MirWord::from_ast_word(Word::literal(format!("{}{}{}", prefix, current, suffix))));
                            current += step;
                        }
                    } else {
                        ...
                        if let (Some(start_char), Some(end_char)) = (range.start.chars().next(), range.end.chars().next()) {
                            ...
                            while if step > 0 { current <= end_code } else { current >= end_code } {
                                if let Some(c) = char::from_u32(current as u32) {
                                    expanded_words.push(MirWord::from_ast_word(Word::literal(format!("{}{}{}", prefix, c, suffix))));
                                }
                                current += step;
                            }
                        }
                ...
                BraceItem::Sequence(seq) => {
                    ...
                    for item in seq {
                        expanded_words.push(MirWord::from_ast_word(Word::literal(format!("{}{}{}", prefix, item, suffix))));
                    }
                }
            }
        } else {
            ...
            Command::Simple(simple_cmd) => {
                ...
                for arg in &simple_cmd.args {
                    match arg {
                        ...
                        _ => {
                            expanded_args.push(MirWord::from_ast_word(arg.clone()));
                        }
                    }
                }
                
                // Create a new MirSimpleCommand with expanded arguments
                let expanded_simple_cmd = MirSimpleCommand {
                    name: MirWord::from_ast_word(simple_cmd.name.clone()),
                    args: expanded_args,
                    redirects: simple_cmd.redirects.clone(),
                    env_vars: simple_cmd.env_vars.iter().map(|(k, v)| (k.clone(), MirWord::from_ast_word(v.clone()))).collect(),
            ...
            Command::For(for_loop) => {
                ...
                for item in &for_loop.items {
                    match item {
                        ...
                        _ => {
                            expanded_items.push(MirWord::from_ast_word(item.clone()));
                        }
                    }
                }
