...
pub enum BraceItem {
    ...
    Range(BraceRange),
    Sequence(Vec<String>),
    Nested(Box<BraceExpansion>),
    Compound(Vec<BraceItem>),
}
            ...
            Word::BraceExpansion(expansion, _) => {
                ...
                for (i, item) in expansion.items.iter().enumerate() {
                    ...
                    match item {
                        ...
                        BraceItem::Nested(nested) => {
                            result.push('{');
                            result.push_str(&format_brace_items(&nested.items));
                            result.push('}');
                        }
                        BraceItem::Compound(items) => {
                            result.push_str(&format_brace_items(items));
                        }
                    }
                }
...
fn format_brace_items(items: &[BraceItem]) -> String {
    let mut result = String::new();
