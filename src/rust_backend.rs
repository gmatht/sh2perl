...
impl Render {
    ...
    fn expr_num(&mut self, e: &IrExpr) -> String {
        match e {
            ...
            IrExpr::BinOp { lhs, op, rhs }
                if matches!(
                    ...
                    BinOpKind::Eq | BinOpKind::Ne | BinOpKind::Lt | BinOpKind::Gt
                        | BinOpKind::Le | BinOpKind::Ge | BinOpKind::And
                        | BinOpKind::Or | BinOpKind::Not
                ) =>
            {
            ...
            other => {
                format!("{}.trim().parse::<i64>().unwrap_or(0)", self.expr_any(other))
            }
        }
    }
    ...
    fn expr_any(&mut self, e: &IrExpr) -> String {
        match e {
            ...
            IrExpr::BinOp { lhs, op, rhs } if *op == BinOpKind::Pow => {
                self.need_pow = true;
                format!("sh2_pow({}, {}).to_string()", self.expr_num(lhs), self.expr_num(rhs))
            }
            IrExpr::BinOp { lhs, op, rhs } => {
                format!(
                    "({} {} {}).to_string()",
        ...
        match toks.as_slice() {
            [a, op, b] => {
                ...
                if let Some(o) = num_op {
                    ...
                    let va = if na { va } else { format!("{va}.trim().parse::<i64>().unwrap_or(0)") };
                    let vb = if nb { vb } else { format!("{vb}.trim().parse::<i64>().unwrap_or(0)") };
                    Some(format!("({va} {o} {vb})"))
    ...
    fn stmt(&mut self, s: &IrStmt) {
        match s {
            ...
            IrStmt::For { var, iter, body } => {
                ...
                match iter {
                    IrExpr::Array(items) => {
                        ...
                        let items_t = if self.is_num(var) {
                            "i64"
                        } else {
                            "String"
                        };
            ...
            IrStmt::Pipeline { .. } | IrStmt::Die { .. }
            | IrStmt::Warn { .. } | IrStmt::Return(_) | IrStmt::SetChildError(_)
            | IrStmt::Require(_) | IrStmt::RawText(_) | IrStmt::Case { .. }
            | IrStmt::Redirect { .. } | IrStmt::Function { .. } | IrStmt::Subshell(_)
            | IrStmt::Background(_) | IrStmt::Exec { .. } => {
...
mod tests {
    ...
    fn assigns_and_echo() {
        ...
        assert!(out.contains("println!(\"x is {}\", x.to_string());"), "{out}");
    }
