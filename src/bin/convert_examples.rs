...
fn categorize_file(
    filename: &str,
    test_results: &std::collections::HashMap<String, String>,
) -> String {
    ...
    if lower_filename.contains("control_flow")
        || lower_filename.contains("if")
        || lower_filename.contains("loop")
        || lower_filename.contains("function")
        || lower_filename.contains("case")
    {
    ...
    } else if lower_filename.contains("pattern_matching")
        || lower_filename.contains("extglob")
        || lower_filename.contains("nocase")
    {
    ...
    } else if lower_filename.contains("arithmetic")
        || lower_filename.contains("numeric")
        || lower_filename.contains("gcd")
        || lower_filename.contains("fibonacci")
        || lower_filename.contains("factorize")
        || lower_filename.contains("primes")
    {
        "Arithmetic & Math".to_string()
    } else if lower_filename.contains("find") || lower_filename.contains("home") {
        "File Operations".to_string()
    } else if lower_filename.contains("local")
        || lower_filename.contains("subprocess")
        || lower_filename.contains("cd")
    {
        "Shell Operations".to_string()
    } else if lower_filename.contains("hard_to_")
        || lower_filename.contains("complex")
        || lower_filename.contains("nested")
    {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if test_results.is_empty() {
        ...
        println!(
            "📊 Loaded test results: {} categorized examples",
            test_results.len()
        );
    }
    ...
    js_content
        .push_str("// This file contains all examples that were previously embedded in WASM\n");
    ...
    js_content
        .push_str("  return JSON.stringify(Object.entries(examples).map(([name, content]) => ({\n");
    ...
    println!(
        "\nTo replace the original, run: mv {} www/examples.js",
        output_file
    );
