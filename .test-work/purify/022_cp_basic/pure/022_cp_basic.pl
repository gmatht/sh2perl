...
#!/usr/bin/perl
BEGIN { $0 = "/home/runner/work/sh2perl/sh2perl/examples.impurl/022_cp_basic.pl" }
...
if (-f "test_cp_dest.txt") {
    ...
    my $content = __bt(do { open my $fh, '<', 'test_cp_dest.txt' or die 'cat: ' . 'test_cp_dest.txt' . ': ' . $! . "\n"; local $/ = undef; my $chunk = <$fh>; close $fh or die 'cat: close failed: ' . $! . "\n"; $chunk; }
);
