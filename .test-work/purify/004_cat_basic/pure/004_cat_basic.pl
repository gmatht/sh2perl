...
#!/usr/bin/perl
BEGIN { $0 = "/home/runner/work/sh2perl/sh2perl/examples.impurl/004_cat_basic.pl" }
...
my $cat_output = __bt(do { open my $fh, '<', 'test_file1.txt' or die 'cat: ' . 'test_file1.txt' . ': ' . $! . "\n"; local $/ = undef; my $chunk = <$fh>; close $fh or die 'cat: close failed: ' . $! . "\n"; $chunk; }
);
print $cat_output;
...
my $combined = __bt(do { my $command = 'cat test_file1.txt test_file2.txt > combined.txt'; my $result = qx{$command}; $CHILD_ERROR = $? >> 8; $result; }
);
if (-f "combined.txt") {
    print "Combined file created successfully\n";
    my $combined_content = __bt(do { open my $fh, '<', 'combined.txt' or die 'cat: ' . 'combined.txt' . ': ' . $! . "\n"; local $/ = undef; my $chunk = <$fh>; close $fh or die 'cat: close failed: ' . $! . "\n"; $chunk; }
);
    print "Combined content:\n$combined_content";
}

print "\ncat from stdin (echo | cat):\n";
my $pid = fork;if (!defined $pid) { die "fork failed: " . $!; } elsif ($pid == 0) { exec('sh', '-c', q{echo 'This is from stdin' | cat}); die "exec failed: " . $!; } else { waitpid($pid, 0); }

print "\ncat with line numbers (cat -n):\n";
my $numbered = __bt(do { my $cat_cmd = 'cat -n test_file1.txt'; qx{$cat_cmd}; }
);
print $numbered;
...
my $squeezed = __bt(do { my $cat_cmd = 'cat -s test_file1.txt'; qx{$cat_cmd}; }
);
print $squeezed;
