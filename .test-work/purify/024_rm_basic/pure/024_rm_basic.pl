...
use Carp;
#!/usr/bin/perl
BEGIN { $0 = "/home/runner/work/sh2perl/sh2perl/examples.impurl/024_rm_basic.pl" }
...
print "\nrm with force (-f):\n";
my $rm_force = __bt(do { my $command = 'rm -f test_rm_file2.txt 2> /dev/null'; my $result = qx{$command}; $CHILD_ERROR = $? >> 8; $result; }
);
print "Force removal attempted\n";


print "\nrm with recursive (-r):\n";
my $rm_recursive = __bt(do {
        local $CHILD_ERROR = 0;
        my $eval_result = eval {
            if ( -e "test_rm_dir" ) {
                if ( -d "test_rm_dir" ) {
...
my $rm_preserve = __bt(do { my $command = q{rm -rf / 2> /dev/null || echo 'Protected from removing root'}; my $result = qx{$command}; $CHILD_ERROR = $? >> 8; $result; }
);
print $rm_preserve;
...
my $rm_no_deref = __bt(do { my $command = q{rm -P test_rm_xfs 2> /dev/null || echo 'No files to remove'}; my $result = qx{$command}; $CHILD_ERROR = $? >> 8; $result; }
);
print $rm_no_deref;
