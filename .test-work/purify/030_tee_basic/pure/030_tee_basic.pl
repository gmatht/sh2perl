...
if (-f "test_tee_output.txt") {
    ...
    my $file_content = __bt(do { open my $fh, '<', 'test_tee_output.txt' or die 'cat: ' . 'test_tee_output.txt' . ': ' . $! . "\n"; local $/ = undef; my $chunk = <$fh>; close $fh or die 'cat: close failed: ' . $! . "\n"; $chunk; }
);
