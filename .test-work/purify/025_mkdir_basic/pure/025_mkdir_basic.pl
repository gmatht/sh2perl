...
use Carp;
#!/usr/bin/perl
BEGIN { $0 = "/home/runner/work/sh2perl/sh2perl/examples.impurl/025_mkdir_basic.pl" }
...
my $mkdir_error = __bt(do { my $command = 'mkdir test_mkdir_error 2>&1'; my $result = qx{$command}; $CHILD_ERROR = $? >> 8; $result; }
);
