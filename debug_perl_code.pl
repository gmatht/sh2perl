...
do {
    my $output = (defined ($ENV{var} // q{}) && ($ENV{var} // q{}) ne q{} ? ($ENV{var} // q{}) : (defined ($ENV{default} // q{}) && ($ENV{default} // q{}) ne q{} ? ($ENV{default} // q{}) : (defined ($ENV{fallback} // q{}) && ($ENV{fallback} // q{}) ne q{} ? ($ENV{fallback} // q{}) : do { my $_result = ("computed"); $_result; })));
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
...
$CHILD_ERROR = $? >> 8;
print "All done\n";
if ((($var =~ /^[0-9]+$/msx && !($main_exit_code = eval { int($ENV{var} > 0) } // "")) && (-f"$file"))) {
if ((q{} =~ /"value"/msx || !(    $main_exit_code = eval { int(0 > 5) } // ""))) {
...
for (eval { int($i=0) } // ""; eval { int($i<0) } // ""; eval { int($i++) } // "") {
        for (eval { int($j=0) } // ""; eval { int($j<${#array[i][@]}) } // ""; eval { int($j++) } // "") {
if (eval { int($ENV{array}[$i][$j] > $ENV{threshold}) } // "") {
                    $result{"i"} = eval { int( $result[$i] + $ENV{array}[$i][$j] ) } // "";
                }
        }
}
...
if ("$ENV{line}" =~ /^.*\$\(.*\).*$/msx) {
                do {
    my $output = "Contains command substitution: $ENV{line}";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
        ...
        $CHILD_ERROR = 0;
                ...
                do {
    my $output = "Contains parameter expansion: $ENV{line}";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
        ...
        $CHILD_ERROR = 0;
                ...
                do {
    my $output = "Contains arithmetic expansion: $ENV{line}";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
