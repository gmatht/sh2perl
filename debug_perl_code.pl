    ...
    if ( -f q{.} ) {
        ...
        if ( opendir my $dh, q{.} ) {
            while ( my $file = readdir $dh ) {
                push @ls_files_272, $file;
            }
            closedir $dh;
            @ls_files_272 = map { $_->[0] } sort { $a->[1] cmp $b->[1] } map { [ $_, do { (my $s = $_) =~ s{/$}{}msx; $s } ] } @ls_files_272;
        }
    }
    @ls_files_272 = map { my $isdir = (-d $_ || -d ( q{.} . q{/} . $_ )); ($isdir ? 'd ' : '- ') . $_ } @ls_files_272;
    (@ls_files_272 ? join("\n", @ls_files_272) . "\n" : q{});
};
