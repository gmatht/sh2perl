...
my $perl_output = `perl -e 'print "Hello from Perl!\\n"'`;
print $perl_output;
...
print "\nperl with one-liner:\n";
my $perl_oneliner = `perl -ne 'print if /Alice/' test_perl_data.txt`;
print $perl_oneliner;
...
my $perl_process = `perl -F',' -ane 'print "$F[0] is $F[1] years old\\n"' test_perl_data.txt`;
print $perl_process;
...
my $perl_array = `perl -e '@data = qw(1 2 3 4 5); print "Sum: " . (join("+", @data)) . " = " . (eval join("+", @data)) . "\\n"'`;
print $perl_array;
...
my $perl_file = `perl -e 'open(FH, "test_perl_data.txt"); print "File opened successfully\\n"; close(FH)'`;
print $perl_file;
...
print "\nperl with modules:\n";
my $perl_module = `perl -e 'use strict; use warnings; print "Modules loaded\\n"'`;
print $perl_module;
...
my $perl_string = `perl -e 'print "Uppercase: " . uc("hello world") . "\\n"'`;
print $perl_string;
...
my $perl_cond = `perl -e 'my $x = 10; print "x is " . ($x > 5 ? "greater" : "less") . " than 5\\n"'`;
print $perl_cond;
