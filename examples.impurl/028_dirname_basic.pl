...
my $dirname_output = `dirname /path/to/file.txt`;
...
my $dirname_relative = `dirname ../file.txt`;
...
my $dirname_root_file = `dirname /file.txt`;
...
my $dirname_nested = `dirname /a/b/c/d/e/file.txt`;
...
my $dirname_multi = `dirname /usr/local/bin/script.sh`;
