...
my $pipeline_output = `cat test_pipeline.txt | grep 'a' | sort`;
...
my $pipeline_head_tail = `cat test_pipeline.txt | head -5 | tail -3`;
...
my $pipeline_uniq_wc = `cat test_pipeline.txt | sort | uniq | wc -l`;
...
my $pipeline_tail_grep = `cat test_pipeline.txt | tail -5 | grep 'a'`;
...
my $pipeline_error = `cat test_pipeline.txt | grep 'x' | wc -l 2>/dev/null`;
...
if (-f "pipeline_output.txt") {
    ...
    my $output_content = `cat pipeline_output.txt`;
