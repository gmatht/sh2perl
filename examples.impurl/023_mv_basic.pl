...
if (-f "test_mv_dest.txt") {
    ...
    my $content = `cat test_mv_dest.txt`;
...
print "\nmv with force (-f):\n";
my $mv_force = `mv -f test_mv_verbose.txt test_mv_force.txt`;
...
print "\nmv with backup (-b):\n";
my $mv_backup = `mv -b test_mv_interactive.txt test_mv_backup.txt`;
...
my $mv_no_target = `mv -T test_mv_suffix.txt test_mv_no_target.txt`;
...
my $mv_no_clobber = `mv -n test_mv_update.txt test_mv_no_clobber.txt`;
...
my $mv_multi = `mv test_mv_file1.txt test_mv_file2.txt test_mv_dir/`;
