...
print "\nrm with force (-f):\n";
my $rm_force = `rm -f test_rm_file2.txt 2>/dev/null`;
...
print "\nrm with recursive (-r):\n";
my $rm_recursive = `rm -r test_rm_dir`;
if (!-d "test_rm_dir") {
...
my $rm_preserve = `rm --preserve-root -rf / 2>/dev/null || echo "Protected from removing root"`;
print $rm_preserve;
...
my $rm_no_deref = `rm -P test_rm_xfs 2>/dev/null || echo "No files to remove"`;
print $rm_no_deref;
...
my $rm_dir = `rm -d test_rm_empty_dir`;
