...
my $time_multi = `time -f 'Real: %E, User: %U, System: %S' sleep 1 2>&1`;
print $time_multi;
...
print "\ntime with pipe:\n";
my $time_pipe = `time ls | head -5 2>&1`;
print $time_pipe;
...
my $time_redirect = `time ls > /dev/null 2>&1`;
