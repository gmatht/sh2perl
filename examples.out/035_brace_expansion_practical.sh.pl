...
my $__set_e = 0;
our $CHILD_ERROR;
$0 = '035_brace_expansion_practical.sh';
$__set_e = 1;
...
my @ls_files_225 = ();
my $ls_all_found_226 = 1;
my @ls_inputs_227 = ();
my @ls_glob_ls_inputs_227_0 = glob('file_*.txt');
if ( !@ls_glob_ls_inputs_227_0 ) {
    push @ls_inputs_227, 'file_*.txt';
    $ls_all_found_226 = 0;
} else {
    push @ls_inputs_227, @ls_glob_ls_inputs_227_0;
}
my @ls_files_228 = ();
my @ls_dirs_229 = ();
my $ls_show_headers_230 = scalar(@ls_inputs_227) > 1;
for my $ls_item_231 (@ls_inputs_227) {
    if ( -f $ls_item_231 ) {
        push @ls_files_228, $ls_item_231;
    }
    elsif ( -d $ls_item_231 ) {
        push @ls_dirs_229, $ls_item_231;
    }
    else {
        $ls_all_found_226 = 0;
    }
}
@ls_files_228 = sort { $a cmp $b } @ls_files_228;
@ls_dirs_229 = sort { $a cmp $b } @ls_dirs_229;
if (@ls_files_228) {
    push @ls_files_225, join("\n", @ls_files_228);
}
for my $ls_dir_232 (@ls_dirs_229) {
    my @ls_dir_entries_233 = ();
    if ( opendir my $dh, $ls_dir_232 ) {
        while ( my $file = readdir $dh ) {
            next if $file eq q{.} || $file eq q{..} || $file =~ /^[.]/;
            push @ls_dir_entries_233, $file;
        }
        closedir $dh;
        @ls_dir_entries_233 = map { $_->[0] } sort { $a->[1] cmp $b->[1] } map { [ $_, do { (my $s = $_) =~ s{/$}{}; $s } ] } @ls_dir_entries_233;
        if ( $ls_show_headers_230 ) {
            if ( @ls_dir_entries_233 ) {
                push @ls_files_225, $ls_dir_232 . ":\n" . join("\n", @ls_dir_entries_233);
            } else {
                push @ls_files_225, $ls_dir_232 . ':';
            }
        }
        elsif ( @ls_dir_entries_233 ) {
            push @ls_files_225, join("\n", @ls_dir_entries_233);
        }
    }
    else {
