...
my $output_0 = qx{command echo -e "match1\\nmatch2\\nmatch3\\nmatch4" | grep -m 2 match};
chomp $output_0;
print $output_0, "\n";
# Original bash: echo "text with pattern in it" | grep -b "pattern"
my $output_1 = qx{command echo 'text with pattern in it' | grep -b pattern};
chomp $output_1;
print $output_1, "\n";
...
my $output_4 = qx{command grep -Z -l pattern temp_file.txt | tr "\\0" "\\n"};
chomp $output_4;
print $output_4, "\n";
my $output_5 = qx{command echo 'text with pattern in it' | grep --color=always pattern};
chomp $output_5;
print $output_5, "\n";
if ($CHILD_ERROR != 0) {
