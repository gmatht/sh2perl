...
my $output_173 = qx{command echo 'HELLO world' | grep -i hello};
chomp $output_173;
print $output_173, "\n";
# Original bash: echo -e "line1\nline2\nline3" | grep -v "line2"
my $output_174 = qx{command echo -e "line1\\nline2\\nline3" | grep -v line2};
chomp $output_174;
print $output_174, "\n";
# Original bash: echo -e "first\nsecond\nthird" | grep -n "second"
my $output_175 = qx{command echo -e "first\\nsecond\\nthird" | grep -n second};
chomp $output_175;
print $output_175, "\n";
# Original bash: echo -e "match\nno match\nmatch again" | grep -c "match"
my $output_176 = qx{command echo -e "match\\nno match\\nmatch again" | grep -c match};
chomp $output_176;
print $output_176, "\n";
# Original bash: echo "text with pattern123 in it" | grep -o "pattern[0-9]\+"
my $output_177 = qx{command echo 'text with pattern123 in it' | grep -o "pattern[0-9]\\+"};
chomp $output_177;
print $output_177, "\n";
my $matched = do { my $input_data = "test"; my $grep_result_178;
