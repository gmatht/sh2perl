...
$CHILD_ERROR = 0;
    ...
    if ($grep_result_194_1 ne q{}) { $grep_result_194_1 .= "\n"; }
...
for my $i (0..@grep_lines_210-1) {
    if (scalar grep { $_ eq $grep_lines_210[$i] } @grep_filtered_210) {
        $file_counts_210{$grep_filenames_210[$i]}++;
    }
}
$grep_result_210 = q{};
for my $file (sort keys %file_counts_210) {
